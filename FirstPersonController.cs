...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        touchpad = FindAnyObjectByType<SpecialTouchPad>();
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + touchpad.GetHorizontalValue() * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * touchpad.GetVerticalValue();
            }
            else
            {
                // Inverted Y
